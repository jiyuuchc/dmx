...
class UVit(nnx.Module):
    ...
    def __post_init__(self, rngs):
        assert self.conv in ["standard", "eff"]
        ...
        conv_in_features = self.input_shape[-1] * (4 ** self.patch_levels)
        self._conv_in = nnx.Conv(
            conv_in_features, self.model_channels * self.channel_mult[0], 
            (3, 3),
            **kwargs
        )
        ...
        res_factor = 2 ** ( len(self.num_blocks) + self.patch_levels )
        num_tokens = self.input_shape[1] // res_factor * self.input_shape[1] // res_factor
        ...
        self._conv_out = nnx.Conv(
            ...
            self.out_channels * (4 ** self.patch_levels),
            (3, 3),
            **kwargs
        )
    ...
    def __call__ (self, x, noise_labels, class_labels=None, *, deterministic=True, rngs=None):
        ...
        # patcherize input
        if self.patch_levels > 0:
            filt = jw.get_filter_bank("bior2.2")
            kernel_dec, _ = jw.make_kernels(filt, x.shape[-1])
            x = jw.wavelet_dec(x, kernel_dec.astype(x.dtype), levels=self.patch_levels)
