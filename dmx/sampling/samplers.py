...
@partial(nnx.jit, static_argnames="correction")
def sampling_step(model:nnx.Module, latent:ArrayLike, sigma:ArrayLike, sigma_next:ArrayLike, *, correction:bool=True, **kwargs):
    x_hat = latent
    sigma_hat = sigma
    # Euler step.
    d_cur = (x_hat - model(x_hat, sigma_hat, **kwargs)) / sigma_hat
    x_next = x_hat + (sigma_next - sigma_hat) * d_cur

    # Apply 2nd order correction.
    if correction:
        d_prime = (x_next - model(x_next, sigma_next, **kwargs)) / sigma_next
